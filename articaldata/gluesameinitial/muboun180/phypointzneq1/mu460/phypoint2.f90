...
!calculate the physical point rho0
  ...
  real(8) yflow(Nflow)
  real(8) rho0,rho
  real(8) mPion2,mPion,mSigma2,mSigma,mf,mf2
  ...
  !mf2=h + dr1dh*(-kappa + rho) + (dr2dh*(-kappa + rho)**2)/2.!h*sqrt(rho/2.)

  mf=(h + dr1dh*(-kappa + rho) + (dr2dh*(-kappa + rho)**2)/2. + &
        (dr3dh*(-kappa + rho)**3)/6. + (dr4dh*(-kappa + rho)**4)/24. + &
     (dr5dh*(-kappa + rho)**5)/120.)*sqrt(rho/2.)
