...
\end{align}
It follows immediately that the second term on the r.h.s. of \Eq{eq:drhoV} is vanishing for the fixed expansion. Consequently, $\bar \lambda_{n,k}$'s of different orders are decoupled and there is no feedback from the high order coupling to low order flows, which results in excellent convergency in the fixed point expansion.
