...
&-4N_c N_f l^{(F)}_{0}(\bar{m}^{2}_{q,k},\eta_{q,k};T,\mu)\bigg]\,, \label{eq:flowV}
\end{align}
...
\end{align}
\colwjf{The threshold functions $l^{(B)}_{0}$ and $l^{(F)}_{0}$ are presented in \ref{}}, and the anomalous dimensions for the mesons and quark are defined as
\begin{align}
...
\end{align}
Note that $z_\phi \equiv Z_{\phi,k}^{\parallel}/Z_{\phi,k}^{\perp}$ enters into the flow of $V_k(\rho)$ through the mesonic fluctuations as shown in \Eq{eq:flowV}. The transversal anomalous dimension for the  $\pi$-meson is obtained by employing the projection as follows
\begin{align}
...
and we neglect the difference of the anomalous dimension between the $\pi$ and $\sigma$ mesons
...
\begin{align}
  \eta_{q}&(p_0,\bm{p})=\frac{1}{4 Z_{q,k}(p_0,\bm{p})}\nonumber \\[2ex]
          &\times\mathrm{Re}\left[\frac{\partial}{\partial (|\bm{p}|^2)}\mathrm{tr}
            ...
            \delta q(p)}\partial_t \Gamma_k\right)\right)\right]\,.   \label{eq:etapsi}
\end{align}
%
...
&-4(\eta_{q,k}-2)\mathcal{F}_{(3)}(\bar{m}^2_{q,k};T,\mu)\bigg]\Bigg\}\,,   
\end{align} 
...
&\times(1-\frac{1}{5}\eta^{\bot}_{\phi,k})+\colwjf{\frac{N_c\bar{h}^{2}_{k}}{z_\phi}}\mathcal{F}_{(3)}(\bar{m}^{2}_{q,k};T,\mu)(4-\eta_{q,k})\bigg\}   
\end{align} 
...
\begin{align}
  G_\phi(q,\bar{m}^{2}_{\phi,k})&=\frac{1}{z_\phi\tilde{q}^{2}_{0}+1+\bar{m}^{2}_{\phi,k}}\,,\\[2ex] 
  G_q(q,\bar{m}^{2}_{q,k})&=\frac{1}{(\tilde{q}_0+i\tilde{\mu})^2+1+\bar{m}^{2}_{q,k}}\,,
\end{align} 
...
\begin{align}
  \mathcal{B}_{(n)}(\bar{m}^{2}_{\phi,k};T)&=\frac{T}{k}\sum_{n_q}\Big(G_{\phi}(q,\bar{m}^{2}_{\phi,k})\Big)^n\,, \\[2ex]
  \mathcal{F}_{(n)}(\bar{m}^{2}_{q,k};T,\mu)&=\frac{T}{k}\sum_{n_q}\Big(G_q(q,\bar{m}^{2}_{q,k})\Big)^n\,.
\end{align}
After performing the Matsubara sum, one arrives at
\begin{align}
    \mathcal{B}_{(1)}(\bar{m}^{2}_{\phi,k};T)=&\frac{1}{\sqrt{z_\phi(1+\bar{m}^{2}_{\phi,k})}}\bigg(\frac{1}{2}+n_{B}(\bar{m}^{2}_{\phi,k},z_\phi;T)\bigg)\,,
\end{align}
and
\begin{align}
    &\mathcal{F}_{(1)}(\bar{m}^{2}_{q,k};T,\mu)=\frac{1}{2\sqrt{1+\bar{m}^{2}_{q,k}}}\nonumber\\[2ex]
    &\hspace{.3cm}\times\Big(1-n_{F}(\bar{m}^{2}_{q,k};T,\mu)-n_{F}(\bar{m}^{2}_{q,k};T,-\mu)\Big)\,,
\end{align}
with the bosonic and fermionic distribution functions as follows
\begin{align}
  n_B(\bar{m}^{2}_{\phi,k},z_\phi;T)=\frac{1}{\exp\lbrace \frac{1}{T}\frac{k}{z_\phi^{1/2}}\sqrt{1+\bar{m}^{2}_{\phi,k}}
\rbrace-1}\,,
\end{align} 
...
\begin{align}
  n_{B}(m^{2};T)=&\frac{1}{\exp\bigg\{\frac{k}{T}\Big(1+m^{2}\Big)^{1/2}\bigg\}-1}\,.
\end{align}
and
\begin{align}
  &n_{F}(m^{2};T,\mu)\nonumber\\[2ex]
  =&\frac{1}{\exp\bigg\{\frac{1}{T}\Big[k(1+m^{2})^{1/2}-\mu\Big]\bigg\}+1}\,, \label{eq:nF}
\end{align}
